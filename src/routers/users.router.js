...
router.get('/', async (req, res) => {
    try {
        ...
        res.status(500).send({ error: "No se pudo obtener usuarios con mongoose", messages: error });
    }
})
...
router.post('/', async (req, res) => {
    try {
        ...
        console.error("No se pudo crear usuarios con mongoose: " + error);
        res.status(500).send({ error: "No se pudo crear usuarios con mongoose", messages: error });
    }
})
